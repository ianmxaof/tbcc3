...
### Chrome Web Store / public listing copy
...
2. **Launch script (recommended):** `cd tbcc && .\start.ps1` — backend + dashboard; opens the dashboard at http://localhost:5173 in **Brave** if it is installed (otherwise your default browser). Use `-NoOpen` to skip opening a browser; use `-Open` to also open the API docs (Swagger). Use `.\start.ps1 -Full` to also start Redis (Docker) + Celery worker (required for "Post now"). **From the browser extension:** gallery top bar or Quick tools → **Launch full stack** — runs the same `-Full` script via `cd tbcc\tools && .\tbcc-launch-daemon.ps1` (leave that window open; it listens on `http://127.0.0.1:8765`), or falls back to `POST /internal/launch-full-stack` when the API is already up (optional **TBCC internal key** in Extension options if `TBCC_INTERNAL_API_KEY` is set).
