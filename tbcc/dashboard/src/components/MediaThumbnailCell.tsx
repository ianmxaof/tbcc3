  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt += 1) {
        try {
          let res = await withThumbnailSlot(() => fetch(cacheUrl, { signal: ac.signal }));
          if (res.status === 404 && attempt === 0) {
            api.media.warmThumbnails([mediaId]).catch(() => {});
          }
          if (res.status === 404 && attempt + 1 < MAX_ATTEMPTS) {
            const ra = Number(res.headers.get("Retry-After"));
            const delayMs = Math.min(15000, (Number.isFinite(ra) && ra > 0 ? ra : 3) * 1000);
            await sleep(delayMs);
            if (cancelled || ac.signal.aborted) return;
            res = await withThumbnailSlot(() => fetch(cacheUrl, { signal: ac.signal }));
          }
