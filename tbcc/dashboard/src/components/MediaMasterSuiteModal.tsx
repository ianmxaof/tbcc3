...
type Props = {
  ...
  /** Permanently remove this media row from TBCC (optional — when set, shows Remove button). */
  onRemoveFromPool?: (mediaId: number) => Promise<void>;
};

export function MediaMasterSuiteModal({
  items,
  openIndex,
  onClose,
  onIndexChange,
  ...
  return (
    <div
      ref={overlayRef}
      className="fixed inset-0 z-[210] flex flex-col bg-slate-950/95"
      role="dialog"
      aria-modal="true"
      aria-label="Media master suite"
      onClick={(e) => {
        if (e.target === overlayRef.current) onClose();
      }}
    >
      <div className="flex items-center justify-between gap-2 px-3 py-2 bg-slate-900 border-b border-slate-700 text-slate-200 text-sm shrink-0 flex-wrap">
        <span>
          Master suite · {openIndex + 1} / {items.length} · ID {current.id} · {current.media_type || "—"}
        </span>
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {onRemoveFromPool ? (
            <button
              type="button"
              className="px-2 py-1 rounded bg-amber-900/90 hover:bg-amber-800 text-xs disabled:opacity-50"
              disabled={removeBusy}
              title="Deletes this row from TBCC only; Telegram Saved Messages copy is not removed"
              onClick={async () => {
                if (
                  !window.confirm(
                    `Remove media #${current.id} from this pool / TBCC? This does not delete the file in Telegram Saved Messages.`
                  )
                ) {
                  return;
                }
                ...
                setRemoveBusy(true);
                try {
                  await onRemoveFromPool(current.id);
                } catch (e) {
            ...
            >
              {removeBusy ? "Removing…" : "Remove from pool"}
            </button>
          ) : null}
      ...
      {removeError ? (
        <p className="text-center text-amber-200 text-xs py-1 bg-amber-950/50 border-b border-amber-900/60 shrink-0">
          {removeError}
        </p>
      ) : null}
      ...
      <div className="flex-1 min-h-0 flex flex-col lg:flex-row overflow-hidden">
        <div
          ref={fsWrapRef}
          className="flex-1 min-h-[42vh] lg:min-h-0 flex items-center justify-center overflow-auto p-3 border-b lg:border-b-0 lg:border-r border-slate-800"
          ...
          >
            {isVideo ? (
              <video
                ...
                src={fileUrl}
                controls
                className="max-w-[min(100vw,96rem)] max-h-[min(50vh,85vh)] lg:max-h-[min(100vh-8rem,85vh)] rounded shadow-lg"
                playsInline
              ...
              <img
                ...
                src={fileUrl}
                alt=""
                className="max-w-[min(100vw,96rem)] max-h-[min(50vh,85vh)] lg:max-h-[min(100vh-8rem,85vh)] object-contain rounded shadow-lg select-none"
                draggable={false}
