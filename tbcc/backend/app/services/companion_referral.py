...
def maybe_credit_referrer_on_gate_complete(referred_user_id: int) -> dict | None:
    """Grant referrer bonus photos when referred user finishes LV + membership gate."""
    ...
    if bonus <= 0:
        return {"referrer_user_id": referrer_id, "bonus_granted": 0, "referred_user_id": int(referred_user_id)}
