/**
 * TBCC fetlife-suite — Chrome extension content script.
 * No Tampermonkey required. Auto-generated by tbcc/userscripts/scripts/build.mjs
 * Source: packages/fetlife-suite/  v1.7.0
 * Built 2026-07-13T18:55:29.586Z
 */
tbccWaitForModule('fetlife_suite', function () {
...
/* GM storage helpers; localStorage when no GM (Chrome extension); memory last.
 * Cross-tab: BroadcastChannel + window "storage" (+ GM_addValueChangeListener when present).
 */
(function (global) {
  'use strict';
  const S = global.__TBCC_US__.shared;
  const mem = Object.create(null);
  const LS_PREFIX = 'tbcc_us_gm:';
  const BC_NAME = 'tbcc-us-storage-v1';
  const TAB_ID =
    global.__TBCC_US_TAB_ID__ ||
    (global.__TBCC_US_TAB_ID__ = `t${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 8)}`);
  ...
  function notifyChange(key, value) {
    try {
      getBc()?.postMessage({ key, value, tab: TAB_ID, t: Date.now() });
    } catch (_) {
      /* ignore */
    }
  }

  function keyMatches(filter, key) {
  ...
  S.storage = {
    ...
    /**
     * Live updates from other FetLife tabs (and TM scripts). Skips the writing tab's BroadcastChannel echo.
     * @param {string|string[]|null|function} keyFilter
     * @param {(key: string, value: *) => void} handler
     * @returns {() => void} unsubscribe
     */
    subscribe(keyFilter, handler) {
      if (typeof handler !== 'function') return () => {};

      const onBc = (ev) => {
        const data = ev?.data;
        if (!data || !data.key) return;
        if (data.tab === TAB_ID) return;
        if (!keyMatches(keyFilter, data.key)) return;
        try {
      ...
      const onStorage = (ev) => {
        if (!ev || !ev.key || !String(ev.key).startsWith(LS_PREFIX)) return;
        const key = String(ev.key).slice(LS_PREFIX.length);
        if (!keyMatches(keyFilter, key)) return;
        let value;
        try {
          value = ev.newValue == null ? undefined : JSON.parse(ev.newValue);
        } catch (_) {
          value = ev.newValue;
        }
        try {
      ...
      if (typeof GM_addValueChangeListener === 'function') {
        ...
        for (const k of keys) {
          try {
            gmIds.push(
              GM_addValueChangeListener(k, (name, _old, newVal, remote) => {
                if (!remote) return;
                try {
                  handler(name, newVal);
                } catch (_) {
...
(function (global) {
  ...
  S.createFlags = function createFlags(storageKey, defaults) {
    ...
    function hydrate() {
      ...
      const next = { ...defaults, ...(typeof saved === 'object' && saved ? saved : {}) };
...
(function (global) {
  ...
  function mountCatalog(host) {
    ...
    toolbar.style.cssText = 'display:flex;gap:8px;margin:0 0 8px;flex-wrap:wrap';
    toolbar.innerHTML = `
      <button type="button" class="primary" data-act="all-on" style="width:auto;margin:0">All on</button>
      <button type="button" data-act="all-off" style="width:auto">All off</button>`;
    host.appendChild(toolbar);
    const note = document.createElement('p');
    note.className = 'hint';
    note.textContent = 'Unchecked = hidden in your browser feed only (not sent to FetLife).';
    host.appendChild(note);

    for (const cat of core.CATALOG) {
      const h = document.createElement('div');
      h.style.cssText = 'padding:8px 0 4px;font-weight:700;color:#bbb';
  ...
  function destroyFloatingUi() {
    ...
    document.getElementById('tbcc-fl-story-fab')?.remove();
  }
...
(function (global) {
  ...
  function goPlace(placeName, opts) {
    ...
    const syncAsl = opts?.syncAsl !== false;
    if (syncAsl && FL.genderFilter?.saveCfg && FL.genderFilter?.loadCfg) {
...
/* ---- packages/fetlife-suite/features/social-proof.js ---- */
...
/* ---- packages/fetlife-suite/data/privacy-presets.json ---- */
(function (global) {
  'use strict';
  const US = global.__TBCC_US__;
  const FL = (US.fetlife = US.fetlife || {});
...
/* ---- packages/fetlife-suite/data/genders.json ---- */
(function (global) {
  'use strict';
  const US = global.__TBCC_US__;
  const FL = (US.fetlife = US.fetlife || {});
...
(function (global) {
  ...
  function renderAutoFollow(body) {
    ...
    const place = FL.placeNav?.loadCfg?.() || {};
    body.innerHTML = `
      <p class="hint">Clicks Follow on visible cards, scrolls for more. With infinite scroll on, stay on a place kinksters page and scroll the pool.</p>
      <label class="row">Place (city / area)</label>
      <input type="text" data-af="place" placeholder="e.g. Palo Alto" />
      <div style="display:flex;gap:8px;margin-top:8px">
        <button type="button" class="primary" data-af="go-place" style="margin-top:0;flex:1">Open kinksters</button>
        <button type="button" data-af="go-search" style="margin-top:0;flex:0 0 auto;padding:6px 10px;background:#2a2a2a;color:#eee;border:1px solid #444;border-radius:6px;cursor:pointer">Search</button>
      </div>
      <p class="hint" data-af="place-hint" style="margin-top:6px">Uses /p/…/kinksters for that place (default region: California, US). Search opens FetLife search if the slug is wrong.</p>
      <label class="row">Speed</label>
      <select data-af="speed"></select>
      <label class="row"><input type="checkbox" data-af="skipMale" /> Skip filtered profiles (ASL)</label>
    ...
    `;
    body.querySelector('[data-af="place"]').value = place.lastQuery || '';
    ...
    const go = (mode) => {
      const q = body.querySelector('[data-af="place"]').value.trim();
      const hint = body.querySelector('[data-af="place-hint"]');
      if (!q) {
        if (hint) hint.textContent = 'Enter a place name first (e.g. Santa Clara).';
        return;
      }
      FL.placeNav?.saveCfg?.({ lastQuery: q });
      const r = FL.placeNav?.goPlace?.(q, { syncAsl: true, mode });
    ...
    body.querySelector('[data-af="place"]').addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        go('kinksters');
      }
  ...
  function renderGender(body) {
    ...
    const cfg = gf?.loadCfg?.() || {};
    body.innerHTML = `
      <p class="hint">Client-side ASL filter. Matching cards are <b>removed</b> (not just hidden) so the list reflows. Infinite scroll then fills from later pages.</p>
      ...
      <input type="text" data-g="location" placeholder="e.g. Palo Alto (blank = any)" />
      <p class="hint" data-g="status" style="margin-top:8px">Click Re-apply after changing options.</p>
    ...
    body.querySelector('[data-g="location"]').value = cfg.locationInclude != null ? cfg.locationInclude : '';
    const save = () => {
      gf.saveCfg({
        ...
        locationInclude: body.querySelector('[data-g="location"]').value.trim(),
      });
    ...
    body.querySelector('[data-g="location"]').addEventListener('change', save);
...
(function (global) {
  ...
  function boot() {
    ...
    console.info('[TBCC FetLife Suite] v1.7 ready', flags.all());
  }
  ...
  console.info('[TBCC] fetlife-suite extension bundle v1.7.0');
});
