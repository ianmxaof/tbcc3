/**
 * TBCC Erome Enhancer v4.2 — grid sorts, filters, infinite scroll, browse intel.
 * Auto-syncs intel batches to TBCC dashboard when pending queue hits syncCap.
 */
(function () {
  'use strict';
  if (window.__tbccEromeEnhancerLoaded) return;
  window.__tbccEromeEnhancerLoaded = true;
  ...
  const INTEL_PENDING_KEY = 'eromeBrowseIntelPendingSync';
  const TBCC_INTEL_API_DEFAULT = 'http://127.0.0.1:8000/analytics/erome-browse-intel';
  const INTEL_META_DEFAULTS = {
    recordIntel: true,
    maxIntelRows: 5000,
    tbccApiUrl: TBCC_INTEL_API_DEFAULT,
  ...
  let intelMeta = Object.assign({}, INTEL_META_DEFAULTS);
  let intelRowsCache = [];
  let pendingSyncRows = [];
  let intelFlushInFlight = false;
  ...
  function migrateFromLocalStorage(data) {
    ...
    try {
      if (!out[STORAGE_KEY] && localStorage.getItem(STORAGE_KEY)) {
        out[STORAGE_KEY] = JSON.parse(localStorage.getItem(STORAGE_KEY));
      }
      if (!out[VIEWED_KEY] && localStorage.getItem(VIEWED_KEY)) {
        out[VIEWED_KEY] = JSON.parse(localStorage.getItem(VIEWED_KEY));
      }
      if (!out[INTEL_KEY] && localStorage.getItem(INTEL_KEY)) {
        out[INTEL_KEY] = JSON.parse(localStorage.getItem(INTEL_KEY));
      }
      if (!out[INTEL_META_KEY] && localStorage.getItem(INTEL_META_KEY)) {
        out[INTEL_META_KEY] = JSON.parse(localStorage.getItem(INTEL_META_KEY));
      }
    } catch (_) {}
    return out;
  }
  ...
  function saveIntelMeta(meta) {
    intelMeta = Object.assign({}, INTEL_META_DEFAULTS, meta || {});
    void chromeSet({ [INTEL_META_KEY]: intelMeta });
  }
  ...
  function saveIntelRows(rows) {
    ...
    intelRowsCache = rows.slice(-cap);
    void chromeSet({ [INTEL_KEY]: intelRowsCache });
  }

  function persistPendingSync() {
    void chromeSet({ [INTEL_PENDING_KEY]: pendingSyncRows });
  }

  function showIntelSyncToast(message) {
    let el = document.getElementById('tbcc-ee-sync-toast');
    if (!el) {
      el = document.createElement('div');
      el.id = 'tbcc-ee-sync-toast';
      el.style.cssText =
        'position:fixed;bottom:72px;right:20px;background:rgba(46,125,90,.95);color:#fff;' +
        'padding:10px 16px;border-radius:8px;font-size:13px;font-weight:600;z-index:10000;' +
  ...
  function flushIntelSync(opts = {}) {
    if (intelFlushInFlight) return Promise.resolve({ ok: false, skipped: true });
    const batch = opts.allPending
      ? pendingSyncRows.splice(0, pendingSyncRows.length)
      : pendingSyncRows.splice(0, pendingSyncRows.length);
    if (!batch.length) return Promise.resolve({ ok: true, appended: 0 });
    intelFlushInFlight = true;
    const apiUrl = (intelMeta.tbccApiUrl || TBCC_INTEL_API_DEFAULT).trim().replace(/\/$/, '');
    return new Promise((resolve) => {
      chrome.runtime.sendMessage(
        { action: 'tbcc-erome-browse-intel-push', rows: batch, apiUrl },
        (resp) => {
          intelFlushInFlight = false;
          if (chrome.runtime.lastError || !resp || !resp.ok) {
            pendingSyncRows = batch.concat(pendingSyncRows);
            persistPendingSync();
            resolve({ ok: false, error: (resp && resp.error) || chrome.runtime.lastError?.message });
            return;
          }
          persistPendingSync();
          updateIntelCountBadge();
          if (opts.toast !== false) {
            showIntelSyncToast(`TBCC: synced ${resp.appended ?? batch.length} intel row(s)`);
          }
          resolve(resp);
        }
      );
  ...
  function maybeAutoFlushIntel() {
    if (intelMeta.autoSyncEnabled === false) return;
    const cap = Math.max(10, parseInt(String(intelMeta.syncCap || 50), 10) || 50);
    if (pendingSyncRows.length >= cap) void flushIntelSync();
  }

  function queueIntelForSync(row) {
    pendingSyncRows.push(row);
    persistPendingSync();
    maybeAutoFlushIntel();
  }
  ...
  function updateIntelCountBadge(opts = {}) {
    ...
    const pending = pendingSyncRows.length;
    el.textContent = String(count);
    if (wrap) {
      wrap.style.opacity = intelMeta.recordIntel !== false ? '1' : '0.4';
      const syncHint =
        intelMeta.autoSyncEnabled !== false
          ? ` · ${pending}/${intelMeta.syncCap || 50} pending sync`
          : '';
      wrap.title =
        intelMeta.recordIntel !== false
          ? `Browse intel: ${count} rows saved (one per album per day)${syncHint}`
  ...
  async function pushIntelToTbcc() {
    const pending = pendingSyncRows.length;
    const all = loadIntelRows();
    if (!pending && !all.length) {
      alert('No intel rows to push.');
      return;
    }
    if (pending) {
      const resp = await flushIntelSync({ toast: false });
      if (resp.ok) {
        alert(`Pushed ${resp.appended ?? pending} pending row(s) to TBCC dashboard.`);
      } else {
        alert('Push failed: ' + (resp.error || 'unknown error'));
      }
      return;
    }
    const apiUrl = (intelMeta.tbccApiUrl || TBCC_INTEL_API_DEFAULT).trim().replace(/\/$/, '');
    return new Promise((resolve) => {
      chrome.runtime.sendMessage(
        { action: 'tbcc-erome-browse-intel-push', rows: all, apiUrl },
        (resp) => {
          if (chrome.runtime.lastError || !resp || !resp.ok) {
            alert('Push failed: ' + ((resp && resp.error) || chrome.runtime.lastError?.message || 'unknown'));
            resolve();
            return;
          }
          alert(`Pushed ${resp.appended ?? 0} rows (scanned ${resp.scanned ?? all.length})`);
          resolve();
        }
      );
    });
  ...
  // State (settings/viewed hydrated in bootstrapStorage)
  ...
  function injectStyles() {
    if (document.getElementById('tbcc-erome-enhancer-css')) return;
    const link = document.createElement('link');
    link.id = 'tbcc-erome-enhancer-css';
    link.rel = 'stylesheet';
    link.href = chrome.runtime.getURL('styles/erome-enhancer.css');
  ...
  function saveSettings() {
    void chromeSet({ [STORAGE_KEY]: settings });
  }
  function loadViewed() {
    return viewedAlbums;
  }
  function saveViewed() {
    void chromeSet({ [VIEWED_KEY]: viewedAlbums });
  }
...
<div class="settings-section"><div class="section-header"><i class="fa fa-clock-o" style="margin-right:8px;"></i>Video Duration Filter</div><div class="section-content" style="background:#333;padding:20px;border-radius:8px;margin-top:12px;border:1px solid #444;"><div class="form-group" style="margin-bottom:20px;"><label class="control-label" style="font-size:14px;color:#ddd;font-weight:500;"><i class="fa fa-filter" style="margin-right:6px;"></i>Minimum Average Video Duration</label><div style="display:flex;align-items:center;gap:12px;margin-top:8px;"><input type="number" id="minVideoSeconds" class="form-control" min="0" placeholder="0 = disabled" style="flex:1;background:#444;border:1px solid #555;color:#fff;"><span style="color:#888;font-size:13px;white-space:nowrap;font-weight:500;">seconds</span></div><div style="font-size:12px;color:#777;margin-top:8px;line-height:1.4;"><i class="fa fa-info-circle" style="margin-right:5px;"></i>Hide albums where the average video duration is shorter than this</div></div><div style="background:#3a3a3a;padding:12px 15px;border-radius:6px;margin-top:15px;border-left:3px solid #eb6395;"><div style="font-size:12px;color:#999;display:flex;align-items:center;"><i class="fa fa-exclamation-circle" style="margin-right:8px;font-size:14px;"></i><span>Applies to both grid pages and individual album pages</span></div></div><div class="ee-action-buttons"><button id="clearViewed" class="btn btn-default ee-action-btn"><i class="fa fa-trash" style="margin-right:6px;"></i>Clear Viewed</button><button id="resetDurationFilter" class="btn btn-default ee-action-btn"><i class="fa fa-refresh" style="margin-right:6px;"></i>Reset Duration</button></div></div></div></div><div class="modal-footer" style="border-top:1px solid #444;padding:20px 25px;"><button id="saveEnhancer" class="btn btn-primary" style="background:#eb6395 !important;border-color:#eb6395 !important;color:#fff !important;font-weight:600;padding:10px 20px;width:100%;"><i class="fa fa-check" style="margin-right:8px;"></i>Apply Settings</button></div></div></div>`;
    ...
    anchor.addEventListener('click', e => {
      ...
      document.getElementById('autoSyncIntel').checked = intelMeta.autoSyncEnabled !== false;
      document.getElementById('syncCap').value = intelMeta.syncCap || 50;
      document.getElementById('tbccApiUrl').value = intelMeta.tbccApiUrl || TBCC_INTEL_API_DEFAULT;
    ...
    modal.querySelector('#saveEnhancer').addEventListener('click', () => {
      ...
      intelMeta.autoSyncEnabled = document.getElementById('autoSyncIntel').checked;
      intelMeta.syncCap = parseInt(document.getElementById('syncCap').value) || 50;
      intelMeta.tbccApiUrl = document.getElementById('tbccApiUrl').value.trim() || TBCC_INTEL_API_DEFAULT;
  ...
  async function bootstrapStorage() {
    const raw = await chromeGet([STORAGE_KEY, VIEWED_KEY, INTEL_KEY, INTEL_META_KEY, INTEL_PENDING_KEY]);
    const data = migrateFromLocalStorage(raw);
    settings = Object.assign({}, DEFAULTS, data[STORAGE_KEY] || {});
    viewedAlbums = Array.isArray(data[VIEWED_KEY]) ? data[VIEWED_KEY] : [];
    intelRowsCache = Array.isArray(data[INTEL_KEY]) ? data[INTEL_KEY] : [];
    intelMeta = Object.assign({}, INTEL_META_DEFAULTS, data[INTEL_META_KEY] || {});
    if (!intelMeta.tbccApiUrl) intelMeta.tbccApiUrl = TBCC_INTEL_API_DEFAULT;
    pendingSyncRows = Array.isArray(data[INTEL_PENDING_KEY]) ? data[INTEL_PENDING_KEY] : [];
    await chromeSet({
      [STORAGE_KEY]: settings,
      [VIEWED_KEY]: viewedAlbums,
      [INTEL_KEY]: intelRowsCache,
      [INTEL_META_KEY]: intelMeta,
      [INTEL_PENDING_KEY]: pendingSyncRows,
    });
  }
  ...
  function startEnhancer() {
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'hidden' && pendingSyncRows.length) {
        void flushIntelSync({ toast: false });
      }
    });
    if (pendingSyncRows.length >= Math.max(10, parseInt(String(intelMeta.syncCap || 50), 10) || 50)) {
      maybeAutoFlushIntel();
    }
  }

  void bootstrapStorage().then(startEnhancer);
})();