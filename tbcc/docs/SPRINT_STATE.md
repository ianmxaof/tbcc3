...
**Last updated:** 2026-07-28 (loot album delivery restored off the import session; AI lane starvation root-caused)
...
| Item | Owner | Notes |
|------|-------|-------|
| Module A revenue stack | agent + operator | **2026-07-27 — deployed to island.** Week 1–2 (`fb295ff`): VIP reprice $18 floor, `bonus_album_draws` applied, `grant_entitlement` on fulfill, Gumroad→Kit capture. Week 3–4: alembic **105** `income_entries.traffic_source_ref` + `revenue_by_source()`; alembic **106** daily micro-pull + streak (`/daily`, tier ≤2, 7-day → +1 free pull) **ships disabled**; alembic **107** `click_links.source_ref` + `GET /analytics/gate-funnel` (click→touch→revenue joined on source_ref, crawlers excluded); loot-bot **lane relay** promotes 11 lanes from click_only to full attribution; `GET /analytics/companion-margin` reports margin as unknown until the cost basis is set. Migrations applied, stack recreated, 16 wk31 beacons seeded + smoked 16/16. **Operator left to do:** (1) paste the 16 beacon URLs into Linkvertise — beacons get no traffic until then; (2) set `TBCC_COMPANION_UNDRESS_USD_PER_CREDIT` from the plan invoice; (3) watch `attributed_revenue_pct` a week; (4) then `TBCC_LOOT_DAILY_PULL_ENABLED=1`. **Finding:** companion `photos_sold`=0 over 30d — conversion problem outranks margin. Docs: `handoffs/2026-07-27_module-a-stack-architect.md`, `GATE_LINK_AUDIT.md`. |
| Loot album delivery + import starvation | agent | **2026-07-28 — deployed.** Root cause: Celery import workers hold a long-lived SQLite handle on `admin_import.session`, so paid roll albums starved on "database is locked". Loot reads now run on `admin_album.session` (`_loot_telegram_io`, env `TBCC_LOOT_DELIVERY_SESSION`); batch downloads keep partial results and only quarantine Telegram-confirmed-missing refs (infra failures no longer reject live media — 27 wrongly rejected rows restored). Smoke: album of 5 delivered. Second bug: 783 channel imports failed since 07-21 with `RuntimeError: Cannot run the event loop while another loop is running` (re-entrant `_run_on_worker_loop`) — now deferred + re-queued with backoff instead of failing the job. AI lane refilled from survivors (`scripts/refill_ai_lane_from_survivors.py`): 20 local-disk rows + 39 of 60 AI STORAGE refs still live → 68 approved; scheduler #2 unpaused and posted an 11-item AI album. Generalized to `scripts/refill_lanes_from_survivors.py` (depth target, one Telethon probe pass, optional unpause): 6 of 11 lanes were already healthy; topped up ai/big_tits/milf/goon/bop with 86 rows and unpaused goon #47 + bop #79. **Open:** the loop re-entrancy itself is unfixed (retry only); all recovered stock is recycled, so storage topics still need fresh deposits. **Operator-only pauses left:** ABG #9 + #50 (session used from two IPs — Telethon re-login), #54 + #122 (manual BOP incident pause 07-23), #111 (restricted chat). |
