...
| **Tag backfill (search widening)** | `tests/test_tag_backfill.py` | `find_thin_tag_media` selection (thin/empty tags, approved-only, `tag_backfill_done` marker excludes repeats, pool filter); `run_tag_backfill_for_media` never calls gatekeeper/pool-routing (tag-only, no re-decide on settled media) |
| **AOF search (`/find`) + continuation** | `tests/test_aof_content_search.py` | Emoji/lane/tag parsing; tier gates + album size; `search_approved_media` `exclude_ids`/`loosen`; `continue_search` Tier 1 → loosened Tier 2 → on-demand SENT VAULT Tier 3 (`TBCC_AOF_SEARCH_TIER3_VAULT`) chain; `aof_search_session` Redis round-trip for the "still want more?" callback token |
| **AOF search lane-picker button tree** | `tests/test_aof_search_menu.py` | `/searchmenu` + bare `/find` lane grid (webcams excluded — no product SKU); `find:lane:`/`find:browse:`/`find:menu` callback wiring in `build_find_handlers`; `consume_find_pending_lane_text` dispatch/clear for the macro bot's reply-keyboard text handler |
...
| **LLM local index / rotator CLI** | `tests/test_llm_model_index.py`, `tests/test_llm_tui.py`, `tests/test_operator_tui.py`, `tests/test_tbcc_cli_llm_ask.py`, `tests/test_tbcc_cli_llm_keys.py`, `tests/test_zeus_llm.py`, `tests/test_prompt_chain_service.py` | PC-local SQLite provider/model index (`tbcc/.tbcc-run/`), sticky auto-cycling `llm ask`, `ask_with_rotation` shared resolve/cycle-on-quota logic (`llm ask` + operator TUI's Ask pane), stored keys/custom providers, price-aware model pick, textual TUIs, stateless `/zeus/v1/ask`; YAML prompt chains (`run_prompt_chain.py`, `tbcc_cli prompt-chain`) |
...
| **Fapello gallery lightbox** | `node --test tbcc/tools/fapello-enhancer/gallery-core.test.mjs` | Profile path/tile parse, thumb→full still, wheel delta, tri-display pin slots, zoom pan offset (from repo root) |
| **Extension operator kit** | `node --test tbcc/extension/tests/tbcc-operator-kit.test.mjs` | Pin catalog + kit state + snippet tooltip delay normalize (from repo root) |
...
| **Extension download routing** | `node --test tbcc/extension/tests/tbcc-download-router.test.mjs` | Circuit-board rule matching + folder/filename templates + route watermark normalize (from repo root) |
...
| **Erome rotate proxy** | `node --test tbcc/extension/tests/erome-proxy-rotate.test.mjs` | PAC builders + ProxyScrape/Proxifly line parse + off-domain kill-menu ids (from repo root); chrome.proxy apply is SW-only |
| **Erome follow** | `node --test tbcc/extension/tests/erome-follow.test.mjs` | Profile username parse + POST `/user/follow/{key}` URL / 403-unauth vs already-following |
