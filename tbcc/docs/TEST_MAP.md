...
| **LLM local index / rotator CLI** | `tests/test_llm_model_index.py`, `tests/test_llm_tui.py`, `tests/test_tbcc_cli_llm_ask.py`, `tests/test_tbcc_cli_llm_keys.py`, `tests/test_zeus_llm.py` | PC-local SQLite provider/model index (`tbcc/.tbcc-run/`), sticky auto-cycling `llm ask`, stored keys/custom providers, price-aware model pick, textual TUI, stateless `/zeus/v1/ask` |
