...
- The command already applies a polite, domain-aware delay between fetches — short for GitHub/HN, ~20s before each of the 15 reddit.com sources (a live run measured real 429s at 3s spacing; 20s was the hand-verified recovery gap, though not stress-tested across a sustained run of 15 in a row — a source that still 429s degrades to a soft per-source failure, retried automatically next run). A full run legitimately takes 5–7 minutes. Let it finish; do not interrupt or retry.
