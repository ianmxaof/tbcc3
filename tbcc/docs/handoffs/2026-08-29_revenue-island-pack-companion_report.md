# Report: Revenue island ops — pack ladder smoke + companion post-credit keyboard

## Phase 2 — Island deploy + external-stop evidence (2026-08-29)

**Against:** `tbcc/docs/handoffs/2026-08-29_revenue-island-pack-companion.md`, "ACTIVE for /cc-run — Phase 2" block
**Session:** cold-start via `/cc-run` — read `CURRENT_DIRECTIVE.md` + this file's Phase 2 block + Phase 1 section below; no other prior chat used

### Deploy

Used `hot-patch-island.ps1` (2-file slice, per Working agreement's preference over full `deploy-island-live.ps1`) — ran `-WhatIf` first to confirm the exact scp/docker-cp/restart plan, then for real:

```
.\scripts\revenue-island\hot-patch-island.ps1 -RelativePaths @("app/services/traffic_attribution.py","bots/payment_bot.py")
```

- Copied both `b04fe50` files to `/opt/tbcc/backend-src/` on the island and `docker cp`'d into all 4 running containers (`api`, `worker`, `worker_telegram`, `worker_post`).
- Restarted all 4 services — compose reported all 4 `Up` within 1–3s, no crash-loop.
- Deploy timestamp: **2026-08-29T04:28:48Z**. No image rebuild (hot-patch copies source directly into the running containers' `/app` tree) — no new image tag; next full `deploy-island-live.ps1` will bake these into the image layer per that script's normal flow.

### External-stop evidence (P2)

```
curl -sS https://api.powercore.app/health
{"status":"ok","external_payment_orders_impl":"uuid-epo-v2","crypto_auto_checkout":true}
```
Confirmed against the public island endpoint (not just the container-local check the hot-patch script itself already printed) immediately after restart.

### Verification (re-run post-deploy)

```
cd tbcc/backend
py -3.13 -m pytest tests/test_traffic_attribution.py tests/test_companion_reveal_paywall.py -q --tb=short
11 passed
```

### Not claimed

Per judgment_ceiling — **I1/I2/I3/I4 smoke is not self-reported as passed.** No Telegram traffic was generated by this session; the health check and pytest above only prove the code is live and unit-correct, not that a real user sees the fixed VIP routing or that a real touch gets recorded. Operator smoke checklist (unchanged from Phase 1, reproduced here for the reverse path):

1. Fresh test account → companion "📦 5 reveals" (`companion_5`) → confirm `UserFunnelTouch.first_source_ref == "src_companion_pack_5"` for that user.
2. Exhaust free credits → tap "⭐ AOF VIP — skip gates" → confirm the payment bot now shows the VIP catalog with **AOF VIP — Intro Month ($10)** listed first (fresh/eligible buyer) instead of the generic welcome screen.
3. Operator verifies `TBCC_GUMROAD_PRODUCT_MAP` on the island includes a `price:1000` key mapped to the intro SKU (P3) — not run or viewed by this session; no secret values pasted here.

### Files touched this phase

None locally — deploy-only. `git status` before and after: 157 pre-existing dirty-tree entries, unchanged count; no new local modifications from this phase besides this report file.

### Risks / known rough edges

- Hot-patch is container-local, not image-baked — a future `--force-recreate` or full redeploy without re-applying this patch (or a proper `deploy-island-live.ps1` run) will revert these containers to whichever image tag is current. Re-apply this hot-patch after any env-var-driven `--force-recreate` on the island.
- No live Telegram traffic exercised the new code path — pytest + health prove the deploy landed and the unit logic is right, not that the funnel behaves correctly end-to-end under real load.

### Next steps

| What | Unblocks | Reversibility | Evidence |
|------|----------|----------------|----------|
| Cursor `/cc-report` + `/silent-fail` ACK on Phase 2 | operator Telegram smoke steps 1–2 | trivial-revert (hot-patch only, no image change) | this section |
| Operator runs Telegram smoke steps 1–3 above | closes I1/I2/I3/I4 with real evidence | trivial-revert | `UserFunnelTouch` row + screenshot of VIP catalog showing Intro Month first |
| Full `deploy-island-live.ps1` once more Phase 1-adjacent work lands (bakes hot-patch into the image) | protects this fix from a future `--force-recreate` wipe | trivial-revert (standard deploy) | new image tag in deploy script output |

**STOP** — Phase 2 done. Waiting for Cursor `/cc-report` + `/silent-fail` before any Phase 3. Not running Telegram smoke myself (operator-gated).

---

# Phase 1 — Code + pytest (2026-08-28)
