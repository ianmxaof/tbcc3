...
async function PlatformLanding({ platform, title }: { platform: ConnectPlatform; title: string }) {
  const db = await createClient();
  let listings: Awaited<ReturnType<typeof fetchConnectListings>> = [];
  try {
    listings = await fetchConnectListings(db, { platform, limit: 48 });
  } catch {
    listings = [];
  }
...
export default function ConnectSnapchatPage() {
  return <PlatformLanding platform="snapchat" title="Snapchat listings" />;
}
